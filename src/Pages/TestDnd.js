...
const TestDnd = () => {
  ...
  const handleDragEnd = (result) => {
    ...
    let { destination: dest, source } = result;
    ...
    let clonedItem = JSON.parse(
      JSON.stringify(state[source.droppableId].items[source.index])
    );
  ...
  const renderDragCardList = (cardList) => {
    return (
      <div className="col-items-list w-full flex gap-2 flex-col items-center justify-center">
  ...
  const renderData = () => {
    return _.map(state, (status, idx) => {
      return (
        <Droppable key={status.id + idx} droppableId={status.id}>
          {(provided) => {
            return (
              <div
