...
import { to, useSpring, animated, easings } from "@react-spring/web";
const Test = () => {
  // animation
  let springConfig = {
    from: { bottom: -25 },
  ...
  let cardDrag = useRef({ name: "", statusId: "" });
  let cardDragEnter = useRef({ name: "", statusId: "" });
  const handleDragStart = (e, task, idx) => {
    cardDrag.current = { ...cardDrag.current, ...task };
  };

  const handleDragEnter = (e, taskDragEnter, idx) => {
    styles.bottom.set(0);
    ...
    // index card dang keo
    let idxTagDrag = taskListUpdate.findIndex(
      (taskItem) => taskItem.name === cardDrag.current.name
    );

    // index card bi keo
    let idxTagDragEnter = taskListUpdate.findIndex(
      (taskItem) => taskItem.name === taskDragEnter.name
    );
